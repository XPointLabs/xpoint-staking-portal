...
function ExitNodes() {
  ...
  const nodes = useMemo(() => {
    if (
      ...
      // Create an object that maps EIP-2537 G1 bytes to the contractId
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        const key = (
          blsKeyPair.data.startsWith('0x') ? blsKeyPair.data.slice(2) : blsKeyPair.data
        ).toLowerCase();
      ...
      for (const node of data.result) {
        const contractId = result[node.info.bls_public_key.toLowerCase()];
        if (!contractId) {
...
async function ejectNodes({
  ...
  for (let i = 0; i < idsToBoot.length; i += batchSize) {
    ...
    for (const id of array) {
      ...
      logger.info(`Service node ${id} exit initiated`);
    }
    logger.info(`Batch ${batchIndex} exit initiated: ${array}`);
    toast.success(`Batch ${batchIndex} exit initiated: ${array}`);
    batchIndex++;
  }
