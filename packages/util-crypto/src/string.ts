...
function genericHexPubkeyComparison(
  ...
  // Optimized loop for fixed-width hex keys.
...
/**
 * Check BLS public key equality (256 hex chars, optionally 0x prefixed)
 * @param key1 - First BLS public key
 * @param key2 - Second BLS public key
 * @returns True if keys are equal
 *
 * NOTE: This function assumes if the BLS key is not falsy it is a valid BLS key, 256 characters without 0x prefix or 258 with 0x prefix.
 */
export const areBLSKeysEqual = (key1?: BLSPublicKey | null, key2?: BLSPublicKey | null) => {
  return genericHexPubkeyComparison(256, key1, key2);
};
