...
export const encodeBlsPubKey = (hex: string) => {
  if (hex.length !== HEX_BYTES.BLS_KEY_BYTES || !/^[0-9a-fA-F]+$/.test(hex)) {
    throw new Error(
      `BLS Pubkey length is invalid, it must be a ${HEX_BYTES.BLS_KEY_BYTES} byte string`
    );
  }

  return { data: `0x${hex}` as Hex };
};

export const encodeBlsSignature = (hex: string) => {
  if (hex.length !== HEX_BYTES.BLS_SIG_BYTES || !/^[0-9a-fA-F]+$/.test(hex)) {
    throw new Error(
      `BLS Signature length is invalid, it must be a ${HEX_BYTES.BLS_SIG_BYTES} byte string`
    );
  }

  return { signature: `0x${hex}` as Hex };
};
