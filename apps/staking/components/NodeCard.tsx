...
const NodeCard = forwardRef<HTMLDivElement, StakeCardProps>(
  ({ className, variant, loading, children, ...props }, ref) => {
    return (
      <div
        className={cn(
          'relative flex w-full flex-row flex-wrap items-center gap-x-3 gap-y-2 rounded-xl bg-module px-5 py-4 align-middle reduced-motion:transition-none transition-all duration-500 ease-in-out md:px-6 md:py-5',
          className
        )}
        style={{
