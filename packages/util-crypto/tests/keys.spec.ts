...
describe('isBLSPublicKey', () => {
  describe('valid keys', () => {
    test('should accept 256-char hex keys', () => {
      const validKeys = [
        '8'.repeat(256),
        'a1b2c3d4e5f6'.repeat(21) + 'a1b2',
        '0'.repeat(256),
        'f'.repeat(256),
        'A'.repeat(256),
        '1234567890abcdef'.repeat(16),
        'abcdef'.repeat(42) + 'abcd',
      ];
    ...
    test('should handle mixed case keys', () => {
      expect(isBLSPublicKey('8123456789ABCDEF'.repeat(16))).toBe(true);
      expect(isBLSPublicKey('8123456789abcdef'.repeat(16))).toBe(true);
      expect(isBLSPublicKey('8123456789AbCdEf'.repeat(16))).toBe(true);
    });
  });
  ...
  describe('edge cases', () => {
    test('should handle all zeros and all F keys', () => {
      expect(isBLSPublicKey('0'.repeat(256))).toBe(true);
      expect(isBLSPublicKey('f'.repeat(256))).toBe(true);
      expect(isBLSPublicKey('F'.repeat(256))).toBe(true);
    });

    test('should use cache for repeated calls', () => {
      const key = '8123456789abcdef'.repeat(16);
...
describe('performance and caching', () => {
  test('should cache results for all key types', () => {
    ...
    const blsKey = '8123456789abcdef'.repeat(16);
