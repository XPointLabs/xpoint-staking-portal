...
import type { NetworkInfo, Stake, StakeContributor } from '@session/staking-api-js/schema';
...
import { type ForwardedRef, type HTMLAttributes, forwardRef, useMemo } from 'react';
...
const useNodeDates = (
  ...
  currentBlock: number,
  networkTime: number,
  blockTimeMs: number = SESSION_NODE.MS_PER_BLOCK
) => {
  ...
  return useMemo(() => {
    const blockTime = new BlockTimeManager(networkTime, currentBlock, blockTimeMs);
...
type StakedNodeCardProps = HTMLAttributes<HTMLDivElement> & {
  toggleId: string;
  stake: Stake;
...
const StakedNodeCard = forwardRef<HTMLDivElement, StakedNodeCardProps>(StakedNodeCardContent);

// biome-ignore lint/complexity/noExcessiveCognitiveComplexity: TODO: see if it can be further componentised
function StakedNodeCardContent(
  {
    stake,
    hideButton,
  ...
  const stakedNodeDictionary = useTranslations('nodeCard.staked');
  ...
  const {
    stakes: {
      networkTime: fallbackNetworkTime,
      blockHeight: fallbackBlockHeight,
      network: fallbackStakingNetwork,
    },
    network,
    contractNodes,
  } = useUser();
  const networkTime = currentNetworkTime ?? fallbackNetworkTime;
  const blockHeight = currentBlockHeight ?? fallbackBlockHeight;
  const stakingNetwork = networkInfo ?? fallbackStakingNetwork;
  ...
  const blockTimeMs = stakingNetwork?.nettype.toLowerCase().includes('arbitrum')
    ? BLOCK_TIME_MS.ARBITRUM
    : SESSION_NODE.MS_PER_BLOCK;
  ...
  const {
    ...
    liquidationDate,
    smallContributorRequestExitDate,
  } = useNodeDates(stake, blockHeight, networkTime, blockTimeMs);
  ...
  const title = useMemo(() => {
    if (eventState === STAKE_EVENT_STATE.EXIT_REQUESTED && state === STAKE_STATE.RUNNING) {
      return stakedNodeDictionary('state.exitRequested');
    }

    switch (state) {
      case STAKE_STATE.EXITED:
        return stakedNodeDictionary('state.exited');
      case STAKE_STATE.AWAITING_EXIT:
        return stakedNodeDictionary('state.awaitingExit');
      case STAKE_STATE.DEREGISTERED:
        return stakedNodeDictionary('state.deregistered');
      case STAKE_STATE.RUNNING:
        return stakedNodeDictionary('state.running');
      case STAKE_STATE.DECOMMISSIONED:
        return stakedNodeDictionary('state.decommissioned');
      default:
        return stakedNodeDictionary('state.unknown');
    }
  }, [eventState, state, stakedNodeDictionary]);
  ...
  return (
    <StakeCard
      ...
      title={title}
      statusIndicatorColor={getNodeStatus(state, eventState)}
  ...
  if (state === STAKE_STATE.RUNNING) {
    if (stakeRequestingExit) {
      return (
        ...
        >
          <NodeExitRequestedButton disabled forceExpanded={forceExpanded} />
        </Tooltip>
      );
    }
