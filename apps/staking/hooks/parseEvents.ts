...
export function compareEventsDesc(a: ArbitrumEvent, b: ArbitrumEvent) {
  if (a.block === b.block) {
...
export function getLatestEvent(events: Array<ArbitrumEvent>) {
  return events.reduce<ArbitrumEvent | undefined>((latest, event) => {
    if (!latest) {
      return event;
    }
    return compareEventsDesc(event, latest) < 0 ? event : latest;
  }, undefined);
}
