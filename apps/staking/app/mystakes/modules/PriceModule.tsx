...
export default function PriceModule() {
  ...
  const range = useMemo(() => {
    ...
    if (data) {
      ...
      const padding = diff === 0 ? Math.max(range.max * 0.05, 0.01) : diff * 0.25;
      range.max = Math.ceil((range.max + padding) * 100) / 100;
      range.min = Math.max(0, Math.floor((range.min - padding) * 100) / 100);
    }
    return range;
  }, [data]);
