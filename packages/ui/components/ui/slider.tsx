    ...
    >
      <SliderPrimitive.Track className="relative mx-1 h-0.5 w-full grow rounded-full bg-gray-lighter">
        ...
        <div className="absolute right-0 left-0 flex h-full items-center justify-center">
          ...
          />
          <SliderLineCircle
            variant={decimalPercent > 0.25 ? 'blue' : 'grey-lighter'}
            strokeVariant={decimalPercent > 0.25 ? 'blue' : 'grey-lighter'}
            style={{ left: `calc(${25}%)` }}
            data-testid={dataTestIds?.slider25}
          />
          <SliderLineCircle
            variant={decimalPercent > 0.5 ? 'blue' : 'grey-lighter'}
            strokeVariant={decimalPercent > 0.5 ? 'blue' : 'grey-lighter'}
            style={{ left: `calc(${50}% - 8px)` }}
            data-testid={dataTestIds?.slider50}
          />
          <SliderLineCircle
            variant={decimalPercent > 0.75 ? 'blue' : 'grey-lighter'}
            strokeVariant={decimalPercent > 0.75 ? 'blue' : 'grey-lighter'}
      ...
      >
        <svg
          ...
          className="absolute"
        >
          <Circle cx="50%" cy="50%" r={circleRadius + extraRad} variant="blue" />
        </svg>
        <svg
          aria-hidden="true"
          focusable="false"
          ...
          className="absolute"
        >
          <Circle cx="50%" cy="50%" r={circleRadius} strokeWidth={1.125} variant="blue" />
        </svg>
        {showPercent ? (
          <span
            className={cn(
