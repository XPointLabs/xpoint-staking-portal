...
export class BlockTimeManager {
  ...
  constructor(networkTime: number, currentBlock: number, blockTimeMs = SESSION_NODE.MS_PER_BLOCK) {
  ...
  getDateOfBlock(targetBlock: number) {
    return new Date(this.networkTime * 1000 + (targetBlock - this.currentBlock) * this.blockTimeMs);
  }
}
